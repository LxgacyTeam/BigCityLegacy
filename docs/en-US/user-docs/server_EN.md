...
[!warning] At the moment, the `msg` command requires at least one player on the server and sends the message effectively on their behalf, only replacing the nickname with "Server". This is caused by the current chat architecture in the game.
