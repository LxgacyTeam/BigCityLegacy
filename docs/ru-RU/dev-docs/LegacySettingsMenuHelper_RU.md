...
[!warning] Скрывать элементы следует **строго после** регистрации всех собственных, т.к скрытый элемент фактически пропадает из сцены и его больше нельзя клонировать, что может привести к ошибке регистрации.
